    ...
    <div className="bg-[var(--background)] min-h-screen">
      ...
      <section className="relative pt-20 pb-8 overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-[1400px] px-5 sm:px-8 lg:px-10 text-center">
          <h1 className="font-heading text-[2rem] sm:text-[2.5rem] md:text-[3.5rem] lg:text-[4rem] font-black leading-tight text-[var(--text-primary)] tracking-tight mb-3">
            Insights That Help Businesses Grow
          </h1>
          <p className="mx-auto max-w-2xl text-[1.1rem] text-[var(--muted)] mb-8">
          ...
          <div className="mx-auto max-w-xl relative">
            ...
            <input
              ...
              className="w-full h-14 pl-12 pr-4 rounded-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[var(--text-primary)] placeholder:text-[var(--muted)] focus:outline-none focus:border-[var(--accent)] focus:ring-1 focus:ring-[var(--accent)] transition-all backdrop-blur-md shadow-[0_8px_30px_rgba(0,0,0,0.12)]"
            />
          </div>
        </div>
      </section>
          ...
          <button
            ...
            className={`whitespace-nowrap px-4 py-2 rounded-full text-[0.9rem] font-bold transition-colors ${
              selectedCategory === null 
                ? "bg-[var(--accent)] text-white dark:text-black shadow-[0_0_15px_var(--accent-glow)]" 
                : "bg-black/5 dark:bg-white/5 text-[var(--muted)] hover:bg-black/10 dark:hover:bg-white/10"
            }`}
          >
            All Topics
          </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          {filteredBlogs.length === 0 && (
            <div className="col-span-full py-20 text-center">
              <p className="text-[var(--muted)] text-[1.1rem]">No articles found matching your criteria.</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1000px] px-5 sm:px-8 lg:px-10 pb-32">
        <div className="rounded-[24px] border border-black/10 dark:border-white/10 bg-gradient-to-br from-black/5 dark:from-white/5 to-transparent p-8 md:p-12 text-center backdrop-blur-sm relative overflow-hidden">
          ...
          <h2 className="font-heading text-[2rem] font-bold text-[var(--text-primary)] mb-4">Stay updated with digital marketing insights.</h2>
          <p className="text-[var(--muted)] mb-8 max-w-xl mx-auto">Get our latest strategies and tutorials delivered straight to your inbox every week. No spam, just value.</p>
          
          <form className="mx-auto flex max-w-md flex-col gap-3 sm:flex-row">
            <input
              type="email"
              placeholder="Enter your email address"
              className="flex-1 rounded-[12px] border border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/40 px-4 py-3 text-[var(--text-primary)] placeholder:text-[var(--muted)] focus:border-[var(--accent)] focus:outline-none"
              required
            />
            <button
              type="submit"
              className="rounded-[12px] bg-[var(--accent)] px-6 py-3 font-bold text-white dark:text-black transition-transform hover:scale-105 hover:bg-[var(--accent-hover)] shadow-[0_0_15px_var(--accent-glow)] whitespace-nowrap"
            >
              Subscribe
            </button>
          </form>
