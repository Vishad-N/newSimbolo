...
export function CaseStudyClientPage({ study: initialStudy, relatedStudies: initialRelated }: CaseStudyClientPageProps) {
  ...
  const handleTwitterShare = () => {
    const url = window.location.href;
    window.open(
      `https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${encodeURIComponent(study.title)}`,
      "_blank",
      "noopener,noreferrer",
    );
  };
  ...
  const handleGenericShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: study.title, text: study.summary, url });
      } catch {
    ...
    <div className="min-h-screen bg-[var(--background)]">
      ...
      <section className="relative pt-32 pb-24 px-6 overflow-hidden min-h-[70vh] flex flex-col justify-center">
        ...
        <div className="container relative z-10 mx-auto max-w-4xl text-center">
          ...
          <div className="relative w-full max-w-3xl mx-auto aspect-video rounded-2xl overflow-hidden mb-10 border border-white/10 shadow-2xl">
            <Image
      ...
      <section className="px-6 py-20 relative z-10">
        <div className="container mx-auto max-w-4xl">
          ...
          <div className="prose prose-invert prose-lg max-w-none mt-16 prose-headings:font-bold prose-headings:tracking-tight prose-a:text-[var(--primary)]">
            ...
            <div className="text-gray-300 leading-relaxed mb-16">
            ...
            <div className="text-gray-300 leading-relaxed mb-16">
              <ReactMarkdown>{study.strategy}</ReactMarkdown>
            </div>
            ...
            <div className="text-gray-300 leading-relaxed mb-12">
              <ReactMarkdown>{study.execution}</ReactMarkdown>
            </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-6 py-8 border-t border-b border-white/10 mt-16">
            ...
            <div className="flex gap-4">
              <button type="button" onClick={handleLinkedInShare} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:bg-blue-500/20 hover:text-blue-400 transition-colors">
              ...
              <button type="button" onClick={handleTwitterShare} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:bg-sky-500/20 hover:text-sky-400 transition-colors">
                <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
              </button>
              <button type="button" onClick={handleGenericShare} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:bg-white/20 hover:text-white transition-colors">
      ...
      <section className="px-6 py-32 relative overflow-hidden">
        ...
        <div className="container relative z-10 mx-auto max-w-3xl text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/contact" className="w-full sm:w-auto h-14 px-8 rounded-full bg-[var(--primary)] text-black font-bold hover:scale-105 transition-transform flex items-center justify-center gap-2">
              Start Your Project <ArrowRight className="w-5 h-5" />
            </Link>
            <Link href="/contact" className="w-full sm:w-auto h-14 px-8 rounded-full bg-white/5 text-white font-bold hover:bg-white/10 transition-colors border border-white/10 flex items-center justify-center">
