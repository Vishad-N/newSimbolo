...
export function PricingToggle({ value, onChange }: PricingToggleProps) {
  return (
    <div className="mx-auto mt-7 grid h-11 w-full max-w-[330px] grid-cols-2 rounded-full border border-white/10 bg-[var(--surface)]/90 p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_12px_28px_rgba(0,0,0,0.22)]">
      {options.map((option) => {
        ...
        return (
          <button
            ...
            className={cn(
              ...
              selected ? "text-[#ffffff]" : "text-white/85 hover:text-white",
            )}
          >
            {selected && (
              <motion.span
