  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.18, duration: 0.65 }} className="relative min-h-[500px]">
      <div className="absolute inset-0 -translate-x-[5cm] translate-y-[5cm]">
        ...
        <motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 6, ease: "easeInOut" }} className="absolute left-[20%] top-[10%] z-30 hidden md:flex items-center justify-center w-[200px] h-[200px] rounded-full border border-[var(--accent)]/3530 shadow-[0_30px_60px_rgba(0,0,0,0.6)]">
        ...
        {/* Floating Users Icon */}
        <motion.div animate={{ y: [0, -8, 0], rotate: [0, 5, 0] }} transition={{ repeat: Infinity, duration: 5.2, ease: "easeInOut" }} className="absolute left-[40%] top-[2%] z-40 hidden h-14 w-14 place-items-center rounded-full border border-[#3B82F6]/30 bg-[#1E3A8A] text-[#3B82F6] shadow-[0_15px_30px_rgba(30,58,138,0.5)] md:grid">
...
export function AffiliateHero({ benefits }: AffiliateHeroProps) {
  return (
    <section className="grid items-center gap-8 lg:grid-cols-[1.1fr_0.9fr]">
      <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }} className="relative z-10">
        <div className="mb-2 flex items-center gap-2 text-[0.72rem] font-medium text-white/70">
          <Link href="/" className="hover:text-white transition-colors">Home</Link>
          <ChevronRight className="h-3.5 w-3.5" />
          <span className="text-white">Affiliate Program</span>
        </div>
        <div className="mb-2.5 inline-flex h-8 w-fit items-center gap-1.5 rounded-full border border-[var(--accent)]/3530 bg-[var(--accent-glow)] px-3 text-[0.7rem] font-black uppercase text-[var(--accent)] shadow-[0_0_24px_var(--accent-glow)]">
          <BadgePercent className="h-3.5 w-3.5" />
          JOIN SIMBOLO AFFILIATE PROGRAM
        </div>
        <h1 className="font-heading text-[1.8rem] font-bold leading-[1.05] tracking-tight text-white md:text-[2.2rem] lg:text-[2.6rem] xl:text-[clamp(2.4rem,3vw,3.2rem)] max-w-[650px]">
          Earn More by Referring <span className="text-[var(--primary)] block">Digital Success.</span>
        </h1>
        <p className="mt-2.5 max-w-[520px] text-[0.92rem] leading-[1.6] text-white/84">Join our affiliate program and earn attractive commissions by referring businesses to The Simbolo&apos;s digital marketing services.</p>
        <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2">
          {benefits.map((benefit) => (
            <div key={benefit} className="flex items-center gap-1.5 text-[0.78rem] font-medium text-white/80 border border-white/10 rounded-full px-3 py-1 bg-white/5">
 ...
 <Link href="/contact?type=affiliate" className="inline-flex h-11 items-center justify-center gap-2.5 rounded-[8px] bg-[var(--primary)] px-6 text-[0.88rem] font-heading font-semibold normal-case tracking-[0.2px] text-[#ffffff] transition duration-300 hover:bg-[var(--primary-hover)] hover:-translate-y-[2px] hover:shadow-[0_12px_28px_var(--primary-glow)] active:bg-[var(--primary-active)]">
          ...
          <Link href="/login" className="inline-flex h-11 items-center justify-center gap-2.5 rounded-[8px] border border-white/15 px-6 text-[0.88rem] font-heading font-medium text-white transition duration-300 hover:bg-white/5">
