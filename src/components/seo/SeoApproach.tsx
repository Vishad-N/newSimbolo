    ...
    <SectionCard className="h-full overflow-hidden p-5">
      ...
      <div className="mt-5 grid gap-5 md:grid-cols-[1fr_190px] lg:grid-cols-1 xl:grid-cols-[1fr_190px]">
        <div className="relative space-y-4">
          <div className="absolute bottom-5 left-4 top-4 w-px bg-[var(--primary)]/45" />
          {steps.map((step, index) => (
            ...
            >
              <div className="z-10 grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[var(--primary)] text-[var(--background)] shadow-[0_0_20px_var(--primary-glow)]">
        ...
        <motion.div animate={{ y: [0, -8, 0] }} transition={{ repeat: Infinity, duration: 5, ease: "easeInOut" }} className="relative mx-auto grid h-[190px] w-[190px] place-items-center">
          ...
          <div className="absolute h-[86px] w-[86px] rounded-full border-[17px] border-[var(--primary)]/65" />
          <Target className="relative h-[95px] w-[95px] text-[var(--primary-light)] drop-shadow-[0_0_18px_var(--primary-glow)]" />
        </motion.div>
      </div>
    </SectionCard>
  );
