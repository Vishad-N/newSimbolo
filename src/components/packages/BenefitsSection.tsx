...
export function BenefitsSection({ items }: BenefitsSectionProps) {
  return (
    <motion.section
      ...
      className="mt-4 grid overflow-hidden rounded-[10px] border border-white/10 bg-[#07101B]/78 shadow-[0_18px_42px_rgba(0,0,0,0.24)] backdrop-blur-xl md:grid-cols-2 xl:grid-cols-4"
    >
      {items.map((item, index) => {
        ...
        return (
          <div key={item.id} className={cn("flex items-center gap-4 px-8 py-5", index > 0 && "border-t border-white/10 md:border-l md:border-t-0")}>
            <div
              className={cn(
                ...
                item.themeColor === "lime"
                  ? "border-[#B9FF00]/25 bg-[#B9FF00]/13 text-[#B9FF00] shadow-[0_0_28px_rgba(185,255,0,0.14)]"
            ...
            <div>
              <h3 className="text-[0.92rem] font-extrabold text-[#B9FF00]">{item.title}</h3>
