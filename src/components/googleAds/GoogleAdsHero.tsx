...
import { ArrowRight, CheckCircle2, ChevronRight, MessageSquare, Search } from "lucide-react";
  ...
  return (
    <div className="relative min-h-[410px]">
      ...
      <motion.div animate={{ y: [0, -13, 0], rotate: [-2, 2, -2] }} transition={{ repeat: Infinity, duration: 6, ease: "easeInOut" }} className="absolute left-[34%] top-[8%] z-20 h-[230px] w-[230px]">
        <Image src="/assets/logo1.png" alt="The Simbolo mascot" width={230} height={230} className="h-full w-full object-contain drop-shadow-[0_0_34px_rgba(45,212,191,0.24)]" priority />
      </motion.div>
      <motion.div animate={{ y: [0, 12, 0] }} transition={{ repeat: Infinity, duration: 4.8 }} className="absolute left-[23%] top-[28%] z-20 hidden h-[78px] w-[78px] rounded-full border-[10px] border-[var(--secondary)]/55 bg-[var(--background)]/50 shadow-[0_0_28px_rgba(45,212,191,0.2)] md:block" />
    ...
    <section className="grid items-start gap-7 xl:grid-cols-[0.82fr_1.18fr]">
      <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }} className="relative z-10">
        ...
        <h1 className="mt-5 max-w-[660px] text-[clamp(2.1rem,4.3vw,3.75rem)] font-black leading-[1.06] tracking-normal text-white">
          ...
          <span className="text-[var(--primary-light)]">Google Ads</span> Campaigns
        </h1>
        <p className="mt-5 max-w-[565px] text-[1rem] leading-7 text-white/82">We create data-driven Google Ads campaigns that bring quality traffic, generate leads, and maximize your ROI.</p>
        ...
        <div className="mt-7 flex flex-col gap-4 sm:flex-row">
          <Link href="/contact?service=google-ads-audit" className="inline-flex h-12 items-center justify-center gap-3 rounded-[8px] bg-[var(--primary)] px-6 text-[0.88rem] font-black text-white shadow-[0_14px_30px_var(--primary-glow)] transition duration-300 hover:scale-[1.02] hover:bg-[var(--primary-hover)]">
            Get Free Google Ads Audit
            <ArrowRight className="h-4 w-4" />
          </Link>
          <Link href="/contact?service=google-ads-expert" className="inline-flex h-12 items-center justify-center gap-3 rounded-[8px] border border-[var(--primary)]/60 px-6 text-[0.88rem] font-black text-white transition duration-300 hover:bg-[var(--primary-glow)]">
