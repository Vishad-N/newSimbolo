        ...
        {results.map((result, index) => {
          ...
          return (
            <motion.div key={result.id} initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: index * 0.08, duration: 0.38 }} className="text-center">
              <div className="mx-auto grid h-13 w-13 place-items-center rounded-full bg-[var(--primary-glow)] text-[var(--primary-light)]">
                <Icon className="h-5 w-5" />
              </div>
              <div className="mt-4 text-[1.45rem] font-black leading-none text-white">{result.value}</div>
