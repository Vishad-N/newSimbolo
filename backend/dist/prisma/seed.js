...
async function main() {
    ...
    // 2. Seed Roles
    const rolesData = [
        { name: 'Super Admin', slug: 'SUPER_ADMIN', description: 'System root administrator with complete access', type: client_1.RoleTypeEnum.SYSTEM },
        { name: 'Admin', slug: 'ADMIN', description: 'Platform administrator with management access', type: client_1.RoleTypeEnum.SYSTEM },
        { name: 'Content Manager', slug: 'CONTENT_MANAGER', description: 'Manages blogs, case studies, FAQs, and media', type: client_1.RoleTypeEnum.SYSTEM },
        { name: 'Project Manager', slug: 'PROJECT_MANAGER', description: 'Manages client projects, orders, and timelines', type: client_1.RoleTypeEnum.SYSTEM },
        { name: 'Marketing Manager', slug: 'MARKETING_MANAGER', description: 'Manages SEO, marketing campaigns, and analytics', type: client_1.RoleTypeEnum.SYSTEM },
        { name: 'Support Agent', slug: 'SUPPORT', description: 'Handles client support tickets and inquiries', type: client_1.RoleTypeEnum.SYSTEM },
        { name: 'Client', slug: 'CLIENT', description: 'Standard client user purchasing services and viewing projects', type: client_1.RoleTypeEnum.SYSTEM },
        { name: 'Affiliate', slug: 'AFFILIATE', description: 'Affiliate partner referring new clients', type: client_1.RoleTypeEnum.SYSTEM },
        { name: 'Editor', slug: 'EDITOR', description: 'Content writer and draft editor', type: client_1.RoleTypeEnum.SYSTEM },
    ];
    ...
    const adminPermSlugs = Object.keys(createdPermissions).filter((slug) => !['settings.manage', 'roles.manage'].includes(slug));
    await prisma.role.update({
