...
class RegisterDto {
    ...
    static _OPENAPI_METADATA_FACTORY() {
        return { email: { required: true, type: () => String, format: "email" }, password: { required: true, type: () => String, minLength: 8, pattern: "((?=.*\\d)|(?=.*\\W+))(?![.\\n])(?=.*[A-Z])(?=.*[a-z]).*$" }, firstName: { required: true, type: () => String }, lastName: { required: true, type: () => String }, phone: { required: false, type: () => String } };
    }
}
...
__decorate([
    (0, swagger_1.ApiPropertyOptional)({ example: '+1234567890', description: 'Optional phone number' }),
