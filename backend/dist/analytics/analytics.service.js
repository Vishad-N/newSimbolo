...
let AnalyticsService = class AnalyticsService extends base_service_1.BaseService {
    prisma;
    constructor(prisma) {
    ...
    async getAdminAnalytics(query = {}) {
        ...
        const now = new Date();
        const [successfulPayments, paymentCounts, monthlyRevenue, activeClients, newClients, projectsByStatus, ordersByStatus, pendingInvoices, teamTasks, completedProjects, openTickets, resolvedTickets, pageViews, inquiries, serviceGroups, packageGroups,] = await Promise.all([
            this.prisma.payment.aggregate({
                where: { status: client_1.PaymentStatusEnum.SUCCESSFUL, ...createdAtWhere },
                _sum: { amount: true },
                _count: true,
            }),
            ...
            this.prisma.invoice.aggregate({
                where: { deletedAt: null, status: { in: [client_1.InvoiceStatusEnum.SENT, client_1.InvoiceStatusEnum.OVERDUE] } },
            ...
            this.prisma.project.findMany({
                where: {
                    deletedAt: null,
                    status: client_1.ProjectStatusEnum.COMPLETED,
                    startDate: { not: null },
            ...
            this.prisma.supportTicket.count({
                where: { deletedAt: null, status: { in: [client_1.TicketStatusEnum.OPEN, client_1.TicketStatusEnum.IN_PROGRESS] } },
            }),
            this.prisma.supportTicket.findMany({
                where: {
                    deletedAt: null,
                    status: { in: [client_1.TicketStatusEnum.RESOLVED, client_1.TicketStatusEnum.CLOSED] },
                    closedAt: { not: null },
                },
            ...
            this.prisma.order.groupBy({
                ...
                where: {
                    deletedAt: null,
                    serviceId: { not: null },
                    status: { notIn: [client_1.OrderStatusEnum.CANCELLED, client_1.OrderStatusEnum.REFUNDED] },
                },
                _count: true,
                _sum: { netAmount: true },
            }),
        ...
        const successCount = paymentCounts.find((item) => item.status === client_1.PaymentStatusEnum.SUCCESSFUL)?._count ?? 0;
        ...
        const averageProjectCompletionDays = completedProjects.length > 0
            ? Number((completedProjects.reduce((sum, project) => {
                const startDate = project.startDate;
                const endDate = project.actualEndDate;
                return sum + (endDate.getTime() - startDate.getTime()) / 86400000;
            }, 0) / completedProjects.length).toFixed(2))
            : 0;
        const averageTicketResolutionHours = resolvedTickets.length > 0
            ? Number((resolvedTickets.reduce((sum, ticket) => {
            ...
            : 0;
        const serviceIds = serviceGroups.map((item) => item.serviceId).filter((id) => Boolean(id));
        const packageIds = packageGroups.map((item) => item.packageId).filter((id) => Boolean(id));
        const [services, packages, workloadUsers] = await Promise.all([
            ...
            this.prisma.user.findMany({
                where: { id: { in: teamTasks.map((item) => item.assignedToId).filter((id) => Boolean(id)) } },
        ...
        const activeProjectStatuses = [
            client_1.ProjectStatusEnum.PLANNING,
            client_1.ProjectStatusEnum.ACTIVE,
            client_1.ProjectStatusEnum.IN_PROGRESS,
        ];
    ...
    async getClientAnalytics(clientId) {
        const now = new Date();
        const [activeProjects, completedProjects, pendingDeliverables, recentPayments, invoiceSummary, upcomingMeetings, projects, recentActivity,] = await Promise.all([
            this.prisma.project.count({
                where: {
                    clientId,
                    deletedAt: null,
                    status: { in: [client_1.ProjectStatusEnum.PLANNING, client_1.ProjectStatusEnum.ACTIVE, client_1.ProjectStatusEnum.IN_PROGRESS] },
                },
            }),
            this.prisma.project.count({ where: { clientId, deletedAt: null, status: client_1.ProjectStatusEnum.COMPLETED } }),
    ...
    async getKpis(query = {}) {
        ...
        const [orders, clients, completedProjects, allProjects] = await Promise.all([
            ...
            this.prisma.project.count({ where: { deletedAt: null, status: client_1.ProjectStatusEnum.COMPLETED } }),
            this.prisma.project.count({ where: { deletedAt: null } }),
        ]);
        return {
            revenueGrowth: analytics.revenue.trends.length >= 2
                ? this.calculateGrowth(analytics.revenue.trends.at(-2)?.amount ?? 0, analytics.revenue.trends.at(-1)?.amount ?? 0)
                : 0,
