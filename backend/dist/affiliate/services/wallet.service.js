...
let WalletService = class WalletService extends base_service_1.BaseService {
     ...
     */
    async applyWalletMutation(tx, walletId, compute) {
        ...
        const rounded = {
            availableBalance: (0, money_util_1.roundCurrency)(next.availableBalance),
            pendingBalance: (0, money_util_1.roundCurrency)(next.pendingBalance),
            lifetimeEarned: (0, money_util_1.roundCurrency)(next.lifetimeEarned),
            lifetimeWithdrawn: (0, money_util_1.roundCurrency)(next.lifetimeWithdrawn),
        };
        ...
        if (result.count !== 1) {
            throw new common_1.ConflictException('Wallet was modified concurrently');
        }
    ...
    async withOptimisticRetry(work) {
        let lastConflict;
        for (let attempt = 1; attempt <= WalletService_1.MAX_RETRIES; attempt += 1) {
            try {
                return await this.prisma.$transaction((tx) => work(tx));
            }
            catch (error) {
                const isConflict = error instanceof common_1.ConflictException ||
                    (error instanceof client_1.Prisma.PrismaClientKnownRequestError && error.code === 'P2034'); // write conflict / deadlock
                if (!isConflict)
                    throw error;
                lastConflict = error;
                this.logger.warn(`Wallet mutation conflict (attempt ${attempt}/${WalletService_1.MAX_RETRIES}) — retrying`);
            }
        }
        this.logger.error('Wallet mutation exhausted retries', lastConflict);
        throw new common_1.ConflictException('Could not update wallet after retries, please retry the request');
    }
    async getWalletByAffiliateId(affiliateId) {
    ...
    async listTransactions(walletId, page = 1, limit = 20) {
     ...
     */
    async creditCommission(commissionId) {
     ...
     */
    async creditCommissionWithin(tx, commissionId) {
        ...
        if (commission.status === client_1.CommissionStatusEnum.CREDITED) {
            return { credited: false, amount: commission.commissionAmount };
        }
        if (commission.status !== client_1.CommissionStatusEnum.ELIGIBLE &&
            commission.status !== client_1.CommissionStatusEnum.PENDING) {
            throw new common_1.BadRequestException(`Commission ${commissionId} cannot be credited from status ${commission.status}`);
        }
        const wallet = await tx.wallet.findUnique({ where: { affiliateId: commission.affiliateId } });
        if (!wallet)
            throw new common_1.NotFoundException(`Wallet for affiliate ${commission.affiliateId} not found`);
        const amount = (0, money_util_1.roundCurrency)(commission.commissionAmount);
        ...
        await tx.walletTransaction.create({
            data: {
                walletId: wallet.id,
                type: client_1.WalletTransactionTypeEnum.COMMISSION_CREDIT,
                amount,
        ...
        await tx.commission.update({
            ...
            data: { status: client_1.CommissionStatusEnum.CREDITED, creditedAt: new Date() },
        });
     ...
     */
    async reserveWithdrawal(affiliateId, amount, options = {}) {
        const requested = (0, money_util_1.roundCurrency)(amount);
        if (!(requested > 0))
        ...
        if (affiliate.status !== client_1.AffiliateStatusEnum.ACTIVE) {
            throw new common_1.BadRequestException('Withdrawals are only available to active sales employees');
        }
        if (requested < settings.minimumWithdrawalAmount) {
            throw new common_1.BadRequestException(`Minimum withdrawal amount is ₹${settings.minimumWithdrawalAmount}`);
        }
        if (requested > settings.maximumWithdrawalAmount) {
            throw new common_1.BadRequestException(`Maximum withdrawal amount is ₹${settings.maximumWithdrawalAmount}`);
        }
        const payoutMethod = options.payoutMethodId
            ...
            : await this.prisma.employeePayoutMethod.findFirst({
                where: { affiliateId, isDefault: true, status: client_1.PayoutMethodStatusEnum.VERIFIED },
            });
        if (!payoutMethod) {
            throw new common_1.BadRequestException('No verified default payout method found. Add and verify one first.');
        }
        if (payoutMethod.status !== client_1.PayoutMethodStatusEnum.VERIFIED) {
            throw new common_1.BadRequestException('The selected payout method is not verified');
        }
        ...
        const withdrawal = await this.withOptimisticRetry(async (tx) => {
            const { before, after } = await this.applyWalletMutation(tx, wallet.id, (w) => {
                if (w.availableBalance < requested) {
                    throw new common_1.BadRequestException('Insufficient available wallet balance');
                }
                return {
            ...
            const created = await tx.withdrawal.create({
                data: {
                    ...
                    walletId: wallet.id,
                    amount: requested,
                    status: client_1.WithdrawalStatusEnum.PENDING,
            ...
            await tx.walletTransaction.create({
                data: {
                    walletId: wallet.id,
                    type: client_1.WalletTransactionTypeEnum.WITHDRAWAL_HOLD,
                    amount: -requested,
                    balanceBefore: before.availableBalance,
                    balanceAfter: after.availableBalance,
     ...
     */
    async releaseWithdrawalHold(withdrawalId, reason, options = {}) {
        const finalStatus = options.finalStatus ?? client_1.WithdrawalStatusEnum.FAILED;
        const outcome = await this.withOptimisticRetry(async (tx) => {
            const withdrawal = await tx.withdrawal.findUnique({ where: { id: withdrawalId } });
            if (!withdrawal)
                throw new common_1.NotFoundException(`Withdrawal ${withdrawalId} not found`);
            const releasable = [
                client_1.WithdrawalStatusEnum.PENDING,
                client_1.WithdrawalStatusEnum.SCHEDULED,
                client_1.WithdrawalStatusEnum.PROCESSING,
            ...
            const amount = (0, money_util_1.roundCurrency)(withdrawal.amount);
            ...
            await tx.walletTransaction.create({
                data: {
                    ...
                    type: client_1.WalletTransactionTypeEnum.WITHDRAWAL_RELEASE,
                    amount,
                    balanceBefore: before.availableBalance,
                    balanceAfter: after.availableBalance,
                    referenceType: 'withdrawal',
        ...
        await this.auditService.logEvent({
            action: finalStatus === client_1.WithdrawalStatusEnum.CANCELLED ? 'withdrawal.cancelled' : 'withdrawal.failed',
            entityType: 'Withdrawal',
     ...
     */
    async debitWithdrawalOnPaid(withdrawalId, options = {}) {
        const outcome = await this.withOptimisticRetry(async (tx) => {
            const withdrawal = await tx.withdrawal.findUnique({ where: { id: withdrawalId } });
            if (!withdrawal)
                throw new common_1.NotFoundException(`Withdrawal ${withdrawalId} not found`);
            if (withdrawal.status !== client_1.WithdrawalStatusEnum.PROCESSING) {
            ...
            await tx.walletTransaction.create({
                data: {
                    walletId: withdrawal.walletId,
                    type: client_1.WalletTransactionTypeEnum.WITHDRAWAL_DEBIT,
                    amount: -amount,
                    balanceBefore: before.availableBalance,
                    balanceAfter: after.availableBalance,
                    referenceType: 'withdrawal',
            ...
            await tx.withdrawal.update({
                where: { id: withdrawalId },
                data: { status: client_1.WithdrawalStatusEnum.PAID, processedAt: new Date() },
            });
            await tx.affiliate.update({
                where: { id: withdrawal.affiliateId },
        ...
        if (outcome.debited) {
            await this.auditService.logEvent({
                ...
                entityId: withdrawalId,
                oldValue: { status: outcome.previousStatus },
                newValue: { status: client_1.WithdrawalStatusEnum.PAID, razorpayPayoutId: options.razorpayPayoutId },
                userId: options.actorUserId,
            });
        }
     ...
     */
    async applyCommissionReversal(commissionId, reversalAmount, options = {}) {
        const amount = (0, money_util_1.roundCurrency)(reversalAmount);
        const outcome = await this.withOptimisticRetry(async (tx) => {
            const commission = await tx.commission.findUnique({ where: { id: commissionId } });
            if (!commission)
                throw new common_1.NotFoundException(`Commission ${commissionId} not found`);
            ...
            if (commission.status !== client_1.CommissionStatusEnum.CREDITED) {
                ...
                await tx.commission.update({
                    ...
                    data: {
                        status: client_1.CommissionStatusEnum.REVERSED,
                        reversedAt: new Date(),
            ...
            await tx.walletTransaction.create({
                data: {
                    walletId: wallet.id,
                    type: client_1.WalletTransactionTypeEnum.COMMISSION_REVERSAL,
                    amount: -amount,
                    balanceBefore: before.availableBalance,
                    balanceAfter: after.availableBalance,
                    referenceType: 'commission',
     ...
     */
    async reverseWithdrawalPayout(withdrawalId, reason) {
        const outcome = await this.withOptimisticRetry(async (tx) => {
            const withdrawal = await tx.withdrawal.findUnique({ where: { id: withdrawalId } });
            if (!withdrawal)
                throw new common_1.NotFoundException(`Withdrawal ${withdrawalId} not found`);
            if (withdrawal.status === client_1.WithdrawalStatusEnum.REVERSED) {
                return { reversed: false, previousStatus: withdrawal.status };
            }
            const amount = (0, money_util_1.roundCurrency)(withdrawal.amount);
            const wasPaid = withdrawal.status === client_1.WithdrawalStatusEnum.PAID;
            const wasHeld = withdrawal.status === client_1.WithdrawalStatusEnum.PROCESSING ||
                withdrawal.status === client_1.WithdrawalStatusEnum.SCHEDULED ||
                withdrawal.status === client_1.WithdrawalStatusEnum.PENDING;
            if (!wasPaid && !wasHeld) {
                return { reversed: false, previousStatus: withdrawal.status };
            }
            const { before, after } = await this.applyWalletMutation(tx, withdrawal.walletId, (w) => ({
            ...
            await tx.walletTransaction.create({
                data: {
                    walletId: withdrawal.walletId,
                    type: client_1.WalletTransactionTypeEnum.PAYOUT_REVERSAL,
                    amount,
                    balanceBefore: before.availableBalance,
                    balanceAfter: after.availableBalance,
                    referenceType: 'withdrawal',
            ...
            await tx.withdrawal.update({
                where: { id: withdrawalId },
                data: { status: client_1.WithdrawalStatusEnum.REVERSED, failureReason: reason },
            });
            await tx.affiliate.update({
                where: { id: withdrawal.affiliateId },
                data: wasPaid
        ...
        if (outcome.reversed) {
            await this.auditService.logEvent({
                ...
                entityId: withdrawalId,
                oldValue: { status: outcome.previousStatus },
                newValue: { status: client_1.WithdrawalStatusEnum.REVERSED, reason },
            });
        }
