...
export class DashboardService extends BaseService {
  ...
  async getAdminPaymentAnalytics(startDate?: Date, endDate?: Date) {
    const where: any = {};
  ...
  async getAdminPendingInvoices(page = 1, limit = 20) {
    const where = { status: { in: ['SENT' as any, 'OVERDUE' as any] }, deletedAt: null };
  ...
  async getClientBillingDashboard(clientId: string) {
    const [paymentHistory, outstandingInvoices, activeSubscription, unreadNotifications] = await Promise.all([
      ...
      this.prisma.invoice.findMany({
        where: { clientId, status: { in: ['SENT' as any, 'OVERDUE' as any] }, deletedAt: null },
        orderBy: { dueDate: 'asc' },
      }),
  ...
  async getAdminWidgets() {
    ...
    return {
      ...
      aiInsights: insights.map((insight) => JSON.parse(insight.value) as Record<string, unknown>),
      notifications,
  ...
  async getClientWidgets(clientId: string) {
